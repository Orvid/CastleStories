...
private struct DietCompiler {
	...
	private void buildBodyWriter(OutputContext output, int base_level, int start_indent_level)
	{
		...
		for( ; m_lineIndex < lineCount; m_lineIndex++ ){
			...
			if( ln[0] == '-' ){ // embedded D code
				...
				size_t j = 0;
				auto tag = isAlpha(ln[0]) || ln[0] == '/' ? skipIdent(ln, j, "/:-_") : "";
				...
				switch(tag){
					...
					case "block": // Block insertion place
						...
						if( block ){
							debug output.writeString("<!-- using block " ~ ln[6 .. $] ~ " in " ~ curline.file ~ "-->");
							if( block.mode == 1 ){
								// output defaults
							}
							...
							// output defaults
							debug output.writeString("<!-- Default block " ~ ln[6 .. $] ~ " in " ~ curline.file ~ "-->");
						}
						break;
	...
	private void buildHtmlNodeWriter(OutputContext output, ref string tag, in string line, int level, bool has_child_nodes, ref bool prepend_whitespaces)
	{
